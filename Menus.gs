...
function menuIntialize() 
{
  ...
  gameMenu.addItem("New Game", 'menuNewGame').addItem("Play", 'showPersona').addItem("newPlay", 'showDialogue').addItem("Suggestions", 'menuSuggestions');
  ...
  // gameMenu.addItem("Select Persona","showPersona") //.addItem("Text-Based RPG", 'menuTextRPG').addItem("Pen & Paper RPG", 'menuPenRPG').addItem("Choose your own adventure book", 'menuAdventureBook').addItem("point & click-esq game", 'menuClickGame').addItem("table top rpg", 'menuTableRPG').addItem("custom", 'menuModeCustom'));
...
function menuPlay(testing,sytleReview) 
{
  ...
  } else if (gameState.game.activeColumn == 3){
    var description = persona.start
  ...
  else if(testing == 1)
  {
    ...
    gameState = narratorAI(text, gameState, sheet,persona)
    postGameState(gameState,sheet)
  }
  else
  {
    // showDialogue(gameState)
    var result = ui.prompt(
      description,
